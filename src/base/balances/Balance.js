...
export class Balance extends Trackable {
  constructor(name, length = 60) {
    super(name);
    this._length = length;
    this._timer = createTimer(this._name, this._length);
    this._status = true;
    this.registerEvents();
  }
  ...
  registerEvents() {
    eventStream.registerEvent(this.name + "LostBalEvent", this.lost.bind(this));
    eventStream.registerEvent(this.name + "GotBalEvent", this.got.bind(this));
  }
